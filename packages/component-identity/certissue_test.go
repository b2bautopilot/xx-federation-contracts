...
func TestGenerateSelfSignedServerCertificate(t *testing.T) {
	issued, err := componentidentity.GenerateSelfSignedServerCertificate(componentidentity.SelfSignedServerCertificateOptions{
...
func TestGenerateSelfSignedServerCertificate_RequiresSAN(t *testing.T) {
	if _, err := componentidentity.GenerateSelfSignedServerCertificate(componentidentity.SelfSignedServerCertificateOptions{
		CommonName: "builders-control",
	}); err == nil {
...
func TestIssueAgentCertificateCarriesBuildersNetIdentityURI(t *testing.T) {
	...
	identityURI := componentidentity.AgentURI(componentidentity.AgentIdentity{
		TenantID:  "tenant-1",
		ProjectID: "project-1",
		NodeID:    "node-1",
	})
...
func TestIssueServerCertificateValidatesWithDeepCheck(t *testing.T) {
	...
	report, err := componentidentity.CheckCertificateMaterial(componentidentity.CertificateCheckOptions{
		Profile:       componentidentity.CertificateProfileServer,
		CertFile:      certFile,
		KeyFile:       keyFile,
		CAFile:        caFile,
...
func TestGenerateAgentCertificateSigningRequestCarriesIdentityProfile(t *testing.T) {
	...
	request, err := componentidentity.GenerateCertificateSigningRequest(componentidentity.CertificateSigningRequestOptions{
		Profile:    componentidentity.CertificateProfileClient,
		CommonName: "builders-agent node-1",
		URIs:       []string{identityURI},
	})
...
func TestGenerateServerCertificateSigningRequestRequiresDNSSAN(t *testing.T) {
	_, err := componentidentity.GenerateCertificateSigningRequest(componentidentity.CertificateSigningRequestOptions{
		Profile:    componentidentity.CertificateProfileServer,
		CommonName: "builders-control",
	})
	if err == nil {
...
func newSignTestCA(t *testing.T, now time.Time) componentidentity.CertificateAuthority {
	t.Helper()
	ca, err := componentidentity.GenerateCertificateAuthority(componentidentity.CertificateAuthorityOptions{
		CommonName: "builders-net-test-ca",
		NotBefore:  now,
...
func TestSignCertificateRequestRoundTripsThroughVerifier(t *testing.T) {
	...
	uri := componentidentity.AgentURI(componentidentity.AgentIdentity{TenantID: "tenant-1", ProjectID: "project-1", NodeID: "node-1"})
	req, err := componentidentity.GenerateCertificateSigningRequest(componentidentity.CertificateSigningRequestOptions{
		Profile:    componentidentity.CertificateProfileClient,
	...
	issued, err := componentidentity.SignCertificateRequest(req.Request, componentidentity.CertificateSignOptions{
		Profile:   componentidentity.CertificateProfileClient,
		URIs:      []string{uri},
		NotBefore: now,
		TTL:       time.Hour,
	...
	if _, err := componentidentity.VerifyIssuedCertificateForCSR(componentidentity.IssuedCSRVerificationOptions{
		Profile:       componentidentity.CertificateProfileClient,
		CertFile:      certFile,
		CSRFile:       csrFile,
		CAFile:        caFile,
...
func TestSignCertificateRequestStampsServerSideSANIgnoringCSR(t *testing.T) {
	now := time.Unix(1_700_000_000, 0).UTC()
	ca := newSignTestCA(t, now)
	evil := componentidentity.AgentURI(componentidentity.AgentIdentity{TenantID: "tenant-EVIL", ProjectID: "p", NodeID: "n"})
	good := componentidentity.AgentURI(componentidentity.AgentIdentity{TenantID: "tenant-GOOD", ProjectID: "p", NodeID: "n"})
	req, err := componentidentity.GenerateCertificateSigningRequest(componentidentity.CertificateSigningRequestOptions{
		Profile:    componentidentity.CertificateProfileClient,
...
func TestSignCertificateRequestRejectsTamperedCSR(t *testing.T) {
	now := time.Unix(1_700_000_000, 0).UTC()
	ca := newSignTestCA(t, now)
	uri := componentidentity.AgentURI(componentidentity.AgentIdentity{TenantID: "t", ProjectID: "p", NodeID: "n"})
	req, err := componentidentity.GenerateCertificateSigningRequest(componentidentity.CertificateSigningRequestOptions{
		Profile:    componentidentity.CertificateProfileClient,
		CommonName: "csr-agent n",
		URIs:       []string{uri},
	...
	if _, err := componentidentity.SignCertificateRequest(tampered, componentidentity.CertificateSignOptions{
		Profile:   componentidentity.CertificateProfileClient,
		URIs:      []string{uri},
		NotBefore: now,
		TTL:       time.Hour,
...
func TestSignCertificateRequestGuards(t *testing.T) {
	...
	if _, err := componentidentity.SignCertificateRequest(nil, componentidentity.CertificateSignOptions{Profile: componentidentity.CertificateProfileClient, URIs: []string{uri}, CA: ca}); err == nil {
		t.Fatal("nil CSR must error")
	}
	if _, err := componentidentity.SignCertificateRequest(req.Request, componentidentity.CertificateSignOptions{Profile: componentidentity.CertificateProfileClient, URIs: []string{uri}}); err == nil {
		t.Fatal("missing CA must error")
	}
	if _, err := componentidentity.SignCertificateRequest(req.Request, componentidentity.CertificateSignOptions{Profile: componentidentity.CertificateProfileClient, CA: ca}); err == nil {
...
func TestVerifyIssuedCertificateForCSRAcceptsProductionReturnedAgentCertificate(t *testing.T) {
	...
	issuedPEM := issueCertificateFromCSR(t, ca, request.Request, componentidentity.CertificateProfileClient, now, time.Hour, nil)
	certFile := writeTestFile(t, dir, "agent-client.crt", issuedPEM)
	csrFile := writeTestFile(t, dir, "agent-client.csr", request.CSRPEM)
	caFile := writeTestFile(t, dir, "agent-client-ca.crt", ca.CertPEM)

	report, err := componentidentity.VerifyIssuedCertificateForCSR(componentidentity.IssuedCSRVerificationOptions{
		Profile:       componentidentity.CertificateProfileClient,
		CertFile:      certFile,
		CSRFile:       csrFile,
		CAFile:        caFile,
...
func TestVerifyIssuedCertificateForCSRRejectsChangedPublicKey(t *testing.T) {
	...
	other, err := componentidentity.GenerateCertificateSigningRequest(componentidentity.CertificateSigningRequestOptions{
		Profile:    componentidentity.CertificateProfileServer,
		CommonName: "builders-control",
		DNSNames:   []string{"x-builders-net.internal"},
	})
	...
	issuedPEM := issueCertificateFromCSR(t, ca, other.Request, componentidentity.CertificateProfileServer, now, time.Hour, nil)
	...
	_, err = componentidentity.VerifyIssuedCertificateForCSR(componentidentity.IssuedCSRVerificationOptions{
		Profile:       componentidentity.CertificateProfileServer,
		CertFile:      certFile,
		CSRFile:       csrFile,
		CAFile:        caFile,
...
func TestVerifyIssuedCertificateForCSRRejectsMissingRequestedSAN(t *testing.T) {
	...
	issuedPEM := issueCertificateFromCSR(t, ca, request.Request, componentidentity.CertificateProfileServer, now, time.Hour, []string{"other.internal"})
	certFile := writeTestFile(t, dir, "control-server.crt", issuedPEM)
	csrFile := writeTestFile(t, dir, "control-server.csr", request.CSRPEM)
	caFile := writeTestFile(t, dir, "control-server-ca.crt", ca.CertPEM)
...
func issueCertificateFromCSR(t *testing.T, ca componentidentity.CertificateAuthority, request *x509.CertificateRequest, profile componentidentity.CertificateProfile, notBefore time.Time, ttl time.Duration, overrideDNS []string) []byte {
	t.Helper()
	...
	switch profile {
	case componentidentity.CertificateProfileServer:
		template.ExtKeyUsage = []x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth}
	case componentidentity.CertificateProfileClient:
