...
func TestLocalFSWorkloadCAProvider(t *testing.T) {
	...
	provider := componentidentity.NewLocalFSWorkloadCAProvider(ca)

	issued, err := provider.IssueCertificate(
		context.Background(),
		componentidentity.CertificateProfileServer,
		"test-server",
		[]string{"localhost"},
...
func TestVaultWorkloadCAProvider(t *testing.T) {
	...
	provider := componentidentity.NewVaultWorkloadCAProvider(srv.URL, "test-token", "pki", "builders-net", srv.Client())
...
func TestSpireWorkloadCAProvider(t *testing.T) {
	...
	provider := componentidentity.NewSpireWorkloadCAProvider("/tmp/spire-agent.sock", "builders-net", ca)
